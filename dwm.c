...
enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
       ...
       NetNumberOfDesktops, NetCurrentDesktop, NetWMWindowTypeDialog, NetClientList, NetLast }; /* EWMH atoms */
...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
                ...
                x = wbar - wstext;
                drw_rect(drw, x, 0, LSPAD + pushSBRight, bh, 1, 1); x += LSPAD + pushSBRight ; /* to keep left padding clean */
                for (;;) {
...
getsystraywidth()
{
	...
	return w ? w + systrayspacing : -1; //patched 1 -> -1 (edge + border)
}
...
manage(Window w, XWindowAttributes *wa)
{
	...
	if(c->iscentered) {
		...
		c->y = ((c->mon->mh + user_bh) + user_bh - HEIGHT(c)) / 2; //patched for user adjustable bh
	}
	...
	c->y = c->mon->my + ((c->mon->mh + user_bh) - HEIGHT(c)) / 2; //patched for user adjustable bh
...
updatesystray(void)
{
	...
	unsigned int sw = TEXTW(stexts) - lrpad - pushSysTrayRight + systrayspacing; //stextc?
	unsigned int w = 1;
